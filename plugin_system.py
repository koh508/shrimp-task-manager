...
class PluginManager:
    ...
    def execute_plugin(self, plugin_name: str, action: str, *args, **kwargs) -> Optional[Dict[str, Any]]:
        """플러그인 실행"""
    ...
    while True:
        ...
        if choice == "0":
            ...
            if action_choice == 'a':
                title = input("   알림 제목: ")
                message = input("   알림 내용: ")
                result = manager.execute_plugin('notification', 'send_notification', title=title, message=message)
            elif action_choice == 'b':
