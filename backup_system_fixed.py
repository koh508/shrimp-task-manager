...
class BackupManager:
    ...
    def create_backup(self, backup_type: str = "manual", description: str = "") -> Optional[str]:
        ...
        try:
            ...
            # 추가 Python 파일 백업 (선택적)
            python_files = [f for f in os.listdir('.') if f.endswith('.py') and f not in self.backup_files]
    ...
    def restore_backup(self, backup_name: str) -> bool:
        ...
        try:
            ...
            # 현재 파일 비상 백업 생성
            emergency_backup = self.create_backup("emergency_before_restore", f"복원 전 비상 백업 ({backup_name})")
            if emergency_backup:
