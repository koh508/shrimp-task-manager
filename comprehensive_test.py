...
def main():
    ...
    print(f"⚡ 성능 테스트: {'✅' if performance_results else '❌'} {'성공' if performance_results else '실패'}")
    
    # 전체 성공률 계산
    total_tests = file_total + package_total + 3  # 환경, GitHub, 성능
    successful_tests = file_success + package_success + 1 + (1 if github_results else 0) + (1 if performance_results else 0)
