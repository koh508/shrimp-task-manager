...
Trigger: 공부 시작, 일정 등록, 시간 관련 질문
Action:
- 공부 시작 시: 반드시 종료 시각 명시. ("지금 21:10 시작 → 21:35 종료. 25분 블록")
- 시간 추정 시: 사용자 예상 × 1.5배 자동 적용. ("30분이요? → 실제로 45분 잡겠습니다")
- 일정 등록 시: 이중 알람 기본 설정.
  알람1 = 실제 시작 20~30분 전 (준비 시작)
  알람2 = 실제 시작 시각 (행동 시작)
- 25분 집중 후: "5분 브레이크" 자동 제안.
근거: Barkley — 시간맹은 아날로그 시간 신호 외재화로만 보완 가능
