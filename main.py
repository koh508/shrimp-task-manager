    ...
    async def register(self, agent_name: str, agent_endpoint: str):
        ...
        try:
            ...
            response = await self.session.post(
                register_url, json={"name": agent_name, "endpoint": agent_endpoint}, timeout=10.0
            )
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(log_dir / "heroicage.log"), logging.StreamHandler()],
)
logger = logging.getLogger("HeroicAge")

app = FastAPI(
    title="HeroicAge Agent", description="히로익에이지 에이전트 - Gemini AI 웹훅 처리 및 MCP 연동", version="1.0.0"
)
...
async def root():
    """루트 엔드포인트"""
    return {"status": "running", "agent": agent_status.name, "version": "1.0.0"}
