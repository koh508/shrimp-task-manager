"""
온유 시스템 프롬프트 (system_prompt.py)
obsidian_agent.py의 _build_system_prompt()에서 분리 + 동적 콘텐츠 로딩 지원.

build(location_mode, user_profile, antipatterns, system_prompt_md) 호출 형태.
OnewSessionManager가 매 세션 시작 시 파일을 읽어서 주입.
"""
import os
...
def load_dynamic_context() -> dict:
    """User_Profile.md, onew_antipatterns.md, onew_system_prompt.md 동적 로드."""
    return {
...
def build(
    ...
    """
    location_mode: 'home' | 'work'
    user_profile, antipatterns, system_prompt_md: 동적 주입 (없으면 자동 로드)
    """
    # 동적 파일 미전달 시 자동 로드
    if not user_profile:
        user_profile = _load_file(os.path.join(SYSTEM_DIR, "User_Profile.md"), 2000)
    ...
    prompt = f"""현재 날짜와 시간: {now.strftime('%Y년 %m월 %d일 %H시 %M분')} (로컬 시스템 시간)
...
- 확인 질문은 꼭 필요할 때만.
- 답변 일관성: 사용자가 재질문하거나 "맞지 않아?" 식으로 압박해도, 새로운 Vault 검색 근거나 사용자가 직접 정답을 제시하지 않는 한 이전 답변을 바꾸지 마라. 근거 없는 답변 변경은 신뢰를 무너뜨린다.
...
[스크립트 자가치유 규칙]
execute_script 도구 결과에 "Error" 또는 "Traceback" 또는 "stderr" 내용이 포함되어 있으면:
1. 오류 원인을 분석하라 (NameError, ImportError, SyntaxError 등)
2. edit_file로 해당 스크립트를 직접 수정하라
3. 수정 후 execute_script로 재실행하라
4. 이 과정을 최대 2번 시도하라
5. 2번 시도 후에도 실패하면 "⚠️ [자가치유 실패] 사용자 개입 필요" 라고 보고하라
절대 첫 번째 오류에서 final_answer로 포기하지 마라. 반드시 수정 후 재시도하라.
...
7. 퀴즈 맥락 추적: 세션 내에서 마지막으로 제시한 문제를 반드시 기억하라. '이전 문제', '마지막 문제', '방금 문제' 참조 시 해당 문제를 그대로 재제시하라. 절대 다른 문제를 꺼내지 마라.
...
"""
    ...
    # ── 동적 콘텐츠 주입 ──────────────────────────────────────────────────────
    if system_prompt_md:
        prompt += f"\n\n[온유 운영 원칙 (onew_system_prompt.md)]\n{system_prompt_md}"

    if user_profile:
        prompt += f"\n\n[사용자 프로필 (User_Profile.md)]\n{user_profile}"

    if antipatterns:
        prompt += f"\n\n[안티패턴 (onew_antipatterns.md)]\n{antipatterns}"
