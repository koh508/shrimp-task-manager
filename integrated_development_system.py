...
class IntegratedDevelopmentSystem:
    ...
    def list_goals(self):
        ...
        for goal in sorted(self.goals, key=lambda x: x["id"]):
            status_icon = {"pending": "⏳", "in_progress": "🏃‍♂️", "completed": "✅"}.get(
                goal["status"], "❓"
            )

            print(
                f"  {status_icon} [{goal['id']}] {goal['title']} ({goal['priority']}) - {goal['progress']}% "
            )
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            ids.add_goal(
                title, desc, priority if priority in ["high", "medium", "low"] else "medium"
            )
        elif choice == "2":
            ids.list_goals()
        elif choice == "3":
